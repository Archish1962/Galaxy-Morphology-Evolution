"""
Training script for the EfficientNet-B0 Galaxy Classifier.

Trains EfficientNet-B0 on galaxy morphology classification with class-weighted
cross-entropy loss and ReduceLROnPlateau scheduling.

Outputs:
    checkpoints/efficientnet_b0.pth         — Best model weights
    outputs/efficientnet/training_log.csv   — Per-epoch metrics
    outputs/efficientnet/confusion_matrix.png
    outputs/efficientnet/training_curves.png

Usage:
    uv run train_efficientnet.py                        # Auto-detects GPU/CPU
    uv run train_efficientnet.py --epochs 30            # Custom epoch count
    uv run train_efficientnet.py --batch-size 64        # Custom batch size
    uv run train_efficientnet.py --fine-tune            # Unfreeze full network
    uv run train_efficientnet.py --device cuda          # Force GPU
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train EfficientNet-B0 galaxy classifier")
...
def plot_training_curves(log_path, output_path):
    ...
    fig.suptitle("EfficientNet-B0 — Training Curves", fontsize=16, fontweight="bold")
    fig.tight_layout()
...
def main():
    ...
    print("=" * 60)
    print("  EFFICIENTNET-B0 GALAXY CLASSIFIER — TRAINING")
    print("=" * 60)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.state_dict(),
                           os.path.join(CHECKPOINT_DIR, "efficientnet_b0.pth"))
    ...
    model.load_state_dict(torch.load(os.path.join(CHECKPOINT_DIR, "efficientnet_b0.pth"),
    ...
    print(f"  Best checkpoint:    {os.path.join(CHECKPOINT_DIR, 'efficientnet_b0.pth')}")
